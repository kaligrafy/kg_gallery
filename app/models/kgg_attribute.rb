# KggAttribute: Each object represents an attribute that can be associated with a KggObject.
# Atributes and their values are set in the jsonb field in KggObject table

class KggAttribute < ActiveRecord::Base
  
  DATATYPES = ["integer", "float", "string", "boolean", "array", "object"]
  ...
  validates_presence_of :datatype # a json datatype
  #has_many :choices, :class_name => "KggAttributeChoice", :foreign_key => "attribute_sn", :primary_key => "shortname", :dependent => :destroy
